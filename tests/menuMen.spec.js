...
test.describe('Menu/Men', () => {
  ...
  test('Menu/Men available to click, see clothes only for men', async ({
    page
  }) => {
    ...
    await expect(page.locator('[role="heading"]').nth(1)).toBeVisible(
      'My Wish List'
    )
  })

  test("Verify the display of My Wish List on the men's page", async ({
    page
  }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'My Wish List' })
    ).toBeVisible()
  ...
  test("Verify the display of Comparison Products on the men's page", async ({
    page
  }) => {
    await page.getByRole('link', { name: 'Sign In' }).click()
    await page.getByLabel('Email').fill('svetik.buratino@gmail.com')
    ...
    await expect(page.locator('#block-compare-heading')).toHaveText(
      'Compare Products'
    )
